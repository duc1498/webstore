        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box">
                    ...
                    @endif
                        <td>
                            <a href="{{ route('admin.vendor.create') }}" class="btn btn-primary pull-right">Thêm mới</a>
                        </td>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        <table class="table table-bordered">
                            ...
                            @foreach ($vendor as $key => $item)
                                <tr class="item-{{ $item->id }}">
                                    ...
                                    <td>
                                        <a href="{{ route('admin.vendor.edit', ['vendor' => $item->id]) }}"><span
                                                title="Chỉnh sửa" type="button" class="btn btn-flat btn-primary"><i
                                                    class="fa fa-edit"></i></span></a>
                                                    @if ($item->deleted_at == null)
                                                    <span data-id="{{ $item->id }}"
                                                        data-url="{{ route('admin.vendor.destroy', $item->id) }}" title="Xóa"
                                                        class="btn btn-flat btn-danger deleteItem"><i
                                                            class="fa fa-trash"></i></span>
                                                @else
                                                    <span data-id="{{ $item->id }}"
                                                        data-url="{{ route('admin.vendor.restore', $item->id) }}"
